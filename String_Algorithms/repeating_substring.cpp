/*
Problem: Repeating Substring
Link: https://cses.fi/problemset/task/2106/
Author: Tamzid Tariq
Date: 2025-10-08
Status: Accepted ✓
*/

#include <bits/stdc++.h>

typedef long long int ll;
typedef unsigned long long int ull;
using namespace std;
#define endl '\n';
const ll modular = 1e9 + 7;
const double pi = acos(-1.0);
const ll INF = 1e18;

struct sparsetable
{
    vector<vector<ll>> lookup;
    vector<ll> arr;
    ll times = 1, siz;

    void initialize(ll num)
    {
        siz = num;
        arr.resize(num);
        times = log2(num);
        lookup.resize(num, vector<ll>(times + 1));
    }

    ll merge(ll val1, ll val2)
    {
        return max(val1, val2);
    }

    void build(vector<ll> &vec)
    {
        for (ll i = 0; i < siz; i++)
            lookup[i][0] = vec[i];

        for (ll j = 1; j <= times; j++)
        {
            for (ll i = 0; i + (1LL << j) <= siz; i++)
                lookup[i][j] = merge(lookup[i][j - 1], lookup[i + (1LL << (j - 1))][j - 1]);
        }
    }

    ll query(ll lo, ll hi)
    {
        ll j = log2(hi - lo + 1);

        return merge(lookup[lo][j], lookup[hi - (1LL << j) + 1][j]);
    }
};

struct SuffixArray
{
    ll num;
    vector<ll> clas, sorted, temp, lcp;
    string s;
    sparsetable ST;

    void build(string &st)
    {
        s = st;
        s += '$';
        num = s.size();

        clas.resize(num), sorted.resize(num), lcp.resize(num);

        iota(sorted.begin(), sorted.end(), 0LL);

        sort(sorted.begin(), sorted.end(), [&](ll ind1, ll ind2)
             { return s[ind1] < s[ind2]; });

        clas[sorted[0]] = 0;

        for (ll i = 1; i < num; i++)
        {
            clas[sorted[i]] = clas[sorted[i - 1]] + (s[sorted[i]] != s[sorted[i - 1]]);
        }

        ll k = 0;

        while ((1LL << k) < num)
        {
            sort(sorted.begin(), sorted.end(), [&](ll ind, ll ind2)
                 { return (make_pair(clas[ind], clas[(ind + (1LL << k)) % num]) < make_pair(clas[ind2], clas[(ind2 + (1LL << k)) % num])); });

            temp = clas;
            temp[sorted[0]] = 0;
            for (ll i = 1; i < num; i++)
            {
                temp[sorted[i]] = temp[sorted[i - 1]] + (make_pair(clas[sorted[i]], clas[(sorted[i] + (1LL << k)) % num]) != make_pair(clas[sorted[i - 1]], clas[(sorted[i - 1] + (1LL << k)) % num]));
            }

            clas = temp;

            k++;
        }

        k = 0;

        for (ll i = 0; i < num - 1; i++)
        {
            ll pi = clas[i];
            ll j = sorted[pi - 1];

            while (s[i + k] == s[j + k])
                k++;

            lcp[pi] = k;
            k = max(0LL, k - 1);
        }

        ST.initialize(num + 1);
        ST.build(lcp);
    }

    void lcs(string &one, string &two)
    {
        s = one;
        s += '$';
        ll num1 = s.size();
        s += two;
        s += '#';
        num = s.size();

        clas.resize(num), sorted.resize(num), lcp.resize(num);

        iota(sorted.begin(), sorted.end(), 0LL);

        sort(sorted.begin(), sorted.end(), [&](ll ind1, ll ind2)
             { return s[ind1] < s[ind2]; });

        clas[sorted[0]] = 0;

        for (ll i = 1; i < num; i++)
        {
            clas[sorted[i]] = clas[sorted[i - 1]] + (s[sorted[i]] != s[sorted[i - 1]]);
        }

        ll k = 0;

        while ((1LL << k) < num)
        {
            sort(sorted.begin(), sorted.end(), [&](ll ind, ll ind2)
                 { return (make_pair(clas[ind], clas[(ind + (1LL << k)) % num]) < make_pair(clas[ind2], clas[(ind2 + (1LL << k)) % num])); });

            temp = clas;
            temp[sorted[0]] = 0;
            for (ll i = 1; i < num; i++)
            {
                temp[sorted[i]] = temp[sorted[i - 1]] + (make_pair(clas[sorted[i]], clas[(sorted[i] + (1LL << k)) % num]) != make_pair(clas[sorted[i - 1]], clas[(sorted[i - 1] + (1LL << k)) % num]));
            }

            clas = temp;

            k++;
        }

        k = 0;

        for (ll i = 0; i < num - 1; i++)
        {
            ll pi = clas[i];
            ll j = sorted[pi - 1];

            while (s[i + k] == s[j + k])
                k++;

            lcp[pi] = k;
            k = max(0LL, k - 1);
        }

        ll start = 0, len = 0;
        for (ll i = 1; i < num; i++)
        {
            if ((sorted[i] < num1) != (sorted[i - 1] < num1))
            {
                // cout<<lcp[i]<<" ";
                if (lcp[i] > len)
                    start = sorted[i - 1], len = lcp[i];
            }
        }

        cout << s.substr(start, len) << endl;
    }

    ll find(string &given)
    {
        ll len = given.size();

        ll lo = 0, hi = num - 1, got = num;

        while (lo <= hi)
        {
            ll mid = (lo + hi) / 2;

            if (s.substr(sorted[mid], len) == given)
            {
                got = mid;
                hi = mid - 1;
            }
            else if (s.substr(sorted[mid], len) > given)
            {
                hi = mid - 1;
            }
            else
                lo = mid + 1;
        }

        lo = got, hi = num - 1;
        ll times = got - 1;

        while (lo <= hi)
        {
            ll mid = (lo + hi) / 2;

            if (s.substr(sorted[mid], len) == given)
            {
                times = mid;
                lo = mid + 1;
            }
            else if (s.substr(sorted[mid], len) > given)
            {
                hi = mid - 1;
            }
            else
                lo = mid + 1;
        }

        if (times < got)
            return -1;

        return ST.query(got, times) + 1;
    }

    ll different_substring()
    {
        ll total = (ll)(num) * (num - 1LL) / 2LL;

        return (ll)total - (ll)accumulate(lcp.begin(), lcp.end(), 0LL);
    }

    void longestPrefix()
    {
        ll lo = 0, hi = num - 1, got = -1, len = ST.query(0, num - 1);

        while (lo <= hi)
        {
            ll mid = (lo + hi) / 2;

            if (ST.query(lo, mid) == len)
            {
                got = mid;
                hi = mid - 1;
            }
            else
                lo = mid + 1;
        }
        if (!len)
        {
            cout << -1 << endl;
        }
        else
            cout << s.substr(sorted[got], len) << endl;
    }
};

void solve()
{
    string s;
    cin >> s;

    ll siz = s.size();

    SuffixArray SA;
    SA.build(s);

    SA.longestPrefix();
}

signed main()
{
    ios_base::sync_with_stdio(0);
    cin.tie(0);
    cout.tie(0);

    ll n;
    n = 1;
    // cin >> n;
    for (ll i = 1; i <= n; ++i)
    {
        // cout << "Case " << i << ": ";
        solve();
    }
    return 0;
}